...
export default function CartProduct({itemId, itemPic, itemTitle, itemQuantity, itemPrice, updateItemsData}){
    ...
    return <div className="cart-product" id={newId} key={newId}>
        ...
        <div className="cart-product-delete" onClick={() => {deleteItem(newId); updateItemsData()}}>🗑</div>
        <div className="cart-product-minus" onClick={() => {decreaseQuantity(newId); updateItemsData()}}>-</div>
        <div className="cart-product-quantity">{itemQuantity}</div>
        <div className="cart-product-plus" onClick={() => {increaseQuantity(newId); updateItemsData()}}>+</div>
